...
public class ContentsProviderController {
    ...
    public ResponseEntity<ResponseEducation> getEducation(@ModelAttribute SearchEducation searchInfo ){
        return contentsProviderService.educationList(searchInfo);
    }

    @GetMapping(value = "/education/{postNumber}")
    public ResponseEntity<ReposeEducationDetail> getEducation(@PathVariable("postNumber") String postNumber) {
        return contentsProviderService.educationDetail(postNumber);
    }
}
